...
expName = 'stroop_simple_spa'  # from the Builder filename that created this script
...
instr1 = visual.TextStim(win=win, name='instr1',
    text=u'En este juego tienes que presionar una tecla seg\xfan el color que tengan las letras.\n\nVamos a empezar practicando un poco.\n\nPresiona cualquier tecla para seguir',
    font='Arial',
    pos=[0, 0], height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0);

# Initialize components for Routine "assign_colours"
assign_coloursClock = core.Clock()
colour_assignment = visual.TextStim(win=win, name='colour_assignment',
    text=u'Presiona:\n\nIZQUIERDA si las letras son ROJAS,\nDERECHA si las letras son VERDES.\n\nPulsa cualquier tecla para empezar la pr\xe1ctica',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0);
...
# ------Prepare to start Routine "assign_colours"-------
t = 0
assign_coloursClock.reset()  # clock
frameN = -1
continueRoutine = True
# update component parameters for each repeat
key_resp_2 = event.BuilderKeyResponse()
# keep track of which components have finished
assign_coloursComponents = [colour_assignment, key_resp_2]
for thisComponent in assign_coloursComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

# -------Start Routine "assign_colours"-------
while continueRoutine:
    # get current time
    t = assign_coloursClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *colour_assignment* updates
    if t >= 0.0 and colour_assignment.status == NOT_STARTED:
        # keep track of start time/frame for later
        colour_assignment.tStart = t
        colour_assignment.frameNStart = frameN  # exact frame index
        colour_assignment.setAutoDraw(True)
...
# -------Ending Routine "assign_colours"-------
for thisComponent in assign_coloursComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "assign_colours" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()

# set up handler to look after randomisation of conditions etc
practice = data.TrialHandler(nReps=4.0, method='fullRandom', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('conditions_red-green_spa.csv'),
